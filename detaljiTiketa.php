...
require 'emailNaTiket.php';

$tiketId = isset($_GET['id']) ? $_GET['id'] : header("Location: svePrijave.php");
...
<body>
    ...
    <div class="ticket-container2">
        ...
        <form action="preuzmiTiket.php" method="post">
        <input type="hidden" name="tiket_id" value="<?php echo $tiket['id']; ?>">

        <div class="button-group">
            <select name="status" class="form-group">
                <option value="">Definiraj Status...</option>
                <option value="otvoren">Otvoren</option>
                <option value="zatvoren">Zatvoren</option>
                <option value="u tijeku">U tijeku</option>
            </select>

            <button type="submit" class="button">Ažuriraj status</button>
        </div>
    </form>
    <br>
    <form action="posaljiOdgovor.php" method="post">
    <input type="hidden" name="tiket_id" value="<?php echo $tiket['id']; ?>">
    <textarea name="odgovor" class="form-group" rows="10" placeholder="Napišite svoj odgovor ovdje..." style="width: 100%; min-height: 150px;"></textarea>
